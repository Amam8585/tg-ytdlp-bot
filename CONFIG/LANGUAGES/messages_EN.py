...
see <a href="https://t.me/LumeTeam">supported sites list</a>
...
<blockquote expandable>🇮🇷 <b>ربات دانلود ویدیو - راهنما</b>


📥 <b>اصلی:</b>

• هر لینکی بفرستید → ربات آن را دانلود می‌کند  

  <blockquote>ربات به‌صورت خودکار تلاش می‌کند ویدیوها را با yt-dlp و تصاویر را با gallery-dl دانلود کند.</blockquote>

• <code>/audio URL</code> → دانلود صوت

• <code>/link [کیفیت] URL</code> → لینک‌های مستقیم

• <code>/proxy</code> → فعال/غیرفعال کردن پروکسی برای همه دانلودها

• روی ویدیو ریپلای کنید با متن → تغییر کپشن


📋 <b>پلی‌لیست‌ها:</b>

• <code>URL*1*5</code> → دانلود ویدیوهای 1 تا 5

• <code>/vid 3-7 URL</code> → تبدیل به <code>URL*3*7</code>


🍪 <b>کوکی‌ها:</b>

• فایل <code>*.txt</code> را برای ویدیوهای خصوصی آپلود کنید

• <code>/cookie [سرویس]</code> → دانلود کوکی (youtube/tiktok/x/custom)

• <code>/cookie youtube 1</code> → انتخاب منبع با ایندکس (1–N)

• <code>/cookies_from_browser</code> → دریافت کوکی از مرورگر

• <code>/check_cookie</code> → بررسی کوکی
...
• <code>/clean</code> → پاک کردن فایل‌های رسانه‌ای

• <code>/clean all</code> → پاک کردن همه چیز

• <code>/clean cookies/logs/tags/format/split/mediainfo/sub/keyboard</code> → پاک کردن بخشی خاص


⚙️ <b>تنظیمات:</b>

• <code>/settings</code> → باز کردن منوی تنظیمات

• <code>/format</code> → تغییر کیفیت و فرمت

• <code>/split</code> → تقسیم ویدیو به بخش‌ها

• <code>/mediainfo on/off</code> → نمایش اطلاعات فایل

• <code>/nsfw on/off</code> → تار کردن یا نمایش آزاد NSFW

• <code>/tags</code> → مشاهده تگ‌های ذخیره‌شده

• <code>/sub on/off</code> → فعال/غیرفعال کردن زیرنویس

• <code>/keyboard</code> → تغییر کیبورد (OFF/1x3/2x3)


🏷️ <b>تگ‌ها:</b>

• بعد از لینک <code>#تگ1#تگ2</code> اضافه کنید

• تگ‌ها در کپشن ظاهر می‌شوند

• <code>/tags</code> → نمایش همه تگ‌ها


🔗 <b>لینک مستقیم:</b>

• <code>/link URL</code> → دانلود با بهترین کیفیت

• <code>/link [144-4320]/720p/1080p/4k/8k URL</code> → انتخاب کیفیت خاص


⚙️ <b>دستورات سریع:</b>

• <code>/format [144-4320]/720p/1080p/4k/8k/best/ask/id 134</code> → انتخاب کیفیت

• <code>/keyboard off/1x3/2x3/full</code> → تغییر چیدمان کیبورد

• <code>/split 100mb-2000mb</code> → تغییر حجم بخش‌ها

• <code>/subs off/fa/en auto</code> → تغییر زبان زیرنویس

• <code>/list URL</code> → نمایش لیست فرمت‌ها

• <code>/mediainfo on/off</code> → فعال/غیرفعال کردن اطلاعات رسانه
...
• <code>/usage</code> → تاریخچه دانلودها

• <code>/search</code> → جستجو با @vid


🖼 <b>تصاویر:</b>

• <code>URL</code> → دانلود تصویر از لینک

• <code>/img URL</code> → دانلود تصاویر از لینک

• <code>/img 11-20 URL</code> → دانلود بازه مشخص

• <code>/img 11- URL</code> → دانلود از آیتم 11 تا انتها

</blockquote>


👨💻 <i>Channel:</i> <a href="https://t.me/LumeTeam">@LumeTeam</a>
...
• <code>/nsfw off</code> - enable blur
    ...
    # Error messages
    ERROR_CHECK_SUPPORTED_SITES_MSG = "Check <a href='https://t.me/LumeTeam'>here</a> if your site supported"
